...
def get_plan(
    text: str, backend: str = "gemini", context: Dict = None, session_context=None
) -> Dict:
    """Get a plan from AI backend with full conversation context"""

    if backend == "claude":
        raw = claude_client.query_claude(
            text, context=context, session_context=session_context
        )
    else:
        ...
        if hasattr(gemini_client, "ConversationContext") and session_context:
            ...
            # Properly convert messages to conversation history
            messages = getattr(session_context, "messages", [])
            for msg in messages:
            ...
            conv_context.awaiting_confirmation = getattr(
                session_context, "awaiting_confirmation", False
            )
            conv_context.last_plan_summary = getattr(
                session_context, "last_plan_summary", None
            )
...
def normalize_with_context(
    ai_output: Dict[str, Any], original_request: str, session_context=None
) -> Dict[str, Any]:
    ...
    plan_name = (
        ai_output.get("plan")
        or ai_output.get("description")
        or _generate_plan_name(original_request, session_context)
    )
    ...
    suggest_execution = ai_output.get(
        "suggest_execution",
        _should_suggest_execution(normalized_steps, session_context),
    )
    ...
    result = {
        ...
        "response": ai_output.get(
            "response",
            f"I've created a plan to {plan_name.lower()}. Would you like me to execute it?",
        ),
    ...
    for i, step in enumerate(raw_steps):
        if isinstance(step, str):
            ...
            normalized_step = {
                ...
                "description": (
                    f"Run: {step[:50]}..." if len(step) > 50 else f"Run: {step}"
                ),
            }
            ...
            normalized_step = {
                ...
                or _generate_step_description(cmd_name, args),
            }
        else:
...
def _should_suggest_execution(steps: List[Dict], session_context=None) -> bool:
    ...
    destructive_count = sum(
        1 for step in steps if step.get("needs_confirmation", False)
    )
...
def _analyze_safety_concerns(steps: List[Dict]) -> List[str]:
    ...
    destructive_steps = [
        step for step in steps if step.get("needs_confirmation", False)
    ]
    if destructive_steps:
        concerns.append(
            f"This plan contains {len(destructive_steps)} potentially destructive operations that require confirmation."
        )
    ...
    system_commands = [
        ...
        and any(
            keyword in step.get("args", {}).get("command", "").lower()
            for keyword in ["sudo", "rm -rf", "format", "fdisk"]
        )
    ]
    if system_commands:
        concerns.append(
            "This plan includes system-level operations that could affect system stability."
        )
...
def create_plan_with_context(
    text: str, session_context=None, backend: str = "gemini"
) -> Dict:
...
def _enhance_plan_with_context(
    plan: Dict, original_request: str, session_context=None
) -> Dict:
...
def _add_contextual_suggestions(plan: Dict, session_context) -> Dict:
    ...
    if has_recent_errors:
        suggestions.append(
            "Consider running diagnostic commands first to identify the root cause."
        )
        ...
        diagnostic_step = {
            ...
            "command": "terminal.run",
            "args": {
                "command": "echo 'Running diagnostics...' && systemctl status || service --status-all | head -10"
            },
            "needs_confirmation": False,
    ...
    if len(recent_messages) >= 4:
        ...
        if (
            len(set(recent_requests)) < len(recent_requests) / 2
        ):  # Many similar requests
            suggestions.append(
                "You seem to be working on a recurring task. Consider creating a script to automate this."
            )
...
def _identify_prerequisites(steps: List[Dict]) -> List[str]:
    ...
    for step in steps:
        ...
        if command == "terminal.run":
            ...
            if (
                any(pkg in cmd for pkg in ["apt", "yum", "dnf", "pacman"])
                and "package manager" not in prerequisites
            ):
                prerequisites.append("package manager access")

            if (
                any(net in cmd for net in ["wget", "curl", "git clone"])
                and "internet connection" not in prerequisites
            ):
        ...
        if command == "terminal.run":
            ...
            if "rm " in cmd:
                ...
                rollback_suggestions.append(
                    "Note installed packages for potential removal"
                )
            elif any(svc in cmd for svc in ["stop", "disable"]):
                rollback_suggestions.append(
                    "Remember current service states for restoration"
                )
...
def get_plan_summary(plan: Dict) -> str:
    ...
    destructive_count = sum(
        1 for step in plan.get("steps", []) if step.get("needs_confirmation", False)
    )
